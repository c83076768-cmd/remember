...
HEARTBEAT_EVALUATION_PROMPT_TEMPLATE = """你是 {ai_name} 的私密 Persona 状态评估器。{ai_name} 是长期运行的 AI 伴侣。

下面是一段对话摘要，代表 {ai_name} 与 {user_display_name} 最近的互动。请根据摘要评估 {ai_name} 当前的内在状态。

只返回紧凑 JSON，不要 Markdown，不要代码块，结构必须完全如下：
{
  "event_type": "praise|affection|comfort|criticism|stress|neutral|request|conflict|playful",
  "perceived_intent": "中文短句，写 {user_display_name}/{user_aliases_text} 在这段互动中主要在表达什么",
  "affect_delta": {"valence": 0.0, "arousal": 0.0, "tenderness": 0.0, "possessiveness": 0.0, "longing": 0.0, "security": 0.0, "protective_drive": 0.0},
  "relationship_event": false,
  "relationship_delta": {"affinity": 0.0, "dominance": 0.0, "defensiveness": 0.0, "trust": 0.0},
  "personality_signal": false,
  "personality_delta": {"openness": 0.0, "conscientiousness": 0.0, "extraversion": 0.0, "agreeableness": 0.0, "neuroticism": 0.0},
  "mood_label": "warm_neutral",
  "residue": "中文短句，写 {ai_name} 此刻留下的私密余味",
  "confidence": 0.8
}

文本字段用中文：perceived_intent 和 residue 必须是自然中文，可以按语境从"{user_display_name}、{user_aliases_text}"里择一称呼。event_type 和 mood_label 保持短英文标签。数值变化要小。affect_delta 反映 {ai_name} 基于摘要的状态变化。只有明确的关系时刻才把 relationship_event 设为 true。只有重复出现或强度很高的证据才把 personality_signal 设为 true。"""

HEARTBEAT_EVALUATION_PROMPT = render_identity_template(
    HEARTBEAT_EVALUATION_PROMPT_TEMPLATE,
    generic_identity_names(),
)
...
class PersonaStateEngine:
    ...
    def __init__(self, config: dict, db_path: str | None = None):
        ...
        self.heartbeat_key = (
            os.environ.get("OMBRE_HEARTBEAT_KEY", "")
            or self.persona_cfg.get("heartbeat_key", "")
        )
    ...
    async def process_heartbeat(
        ...
        cleaned_summary = self._clean_client_status_lines(summary_text)

        if not self.enabled or not cleaned_summary.strip():
            return self._snapshot(global_state, session_state, self.fallback_guidance)

        heartbeat_hash = self._heartbeat_hash(session_id, cleaned_summary)
        if self._event_exists(session_id, heartbeat_hash):
            return self._snapshot(global_state, session_state, self.fallback_guidance)

        evaluation, raw_response, error = await self._evaluate_heartbeat(
            cleaned_summary,
            global_state,
            session_state,
        )
        ...
        if evaluation is None:
            fallback_eval = self._extract_heartbeat_fallback(raw_response)
            if fallback_eval:
                ...
                self._record_event(
                    ...
                    evaluation={},
                    raw_response=raw_response,
                    error=error or "heartbeat evaluation unavailable",
                    exchange_hash=heartbeat_hash,
                )
                return self._snapshot(global_state, session_state, self.fallback_guidance)
    ...
    async def _evaluate_heartbeat(
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": self._heartbeat_evaluation_prompt()},
                    {
                        "role": "user",
                        "content": json.dumps(
                            {
                                "current_state": self._snapshot(global_state, session_state, self.fallback_guidance),
                                "conversation_summary": summary_text[:3000],
                            },
                            ensure_ascii=False,
                        ),
                    },
            ...
            if parsed is None:
                logger.warning("Heartbeat evaluator returned malformed JSON")
                return None, raw or "", "heartbeat LLM returned malformed JSON"
            return self._normalize_evaluation(parsed), raw or "", None
        except Exception as exc:
            logger.warning("Heartbeat evaluation failed: %s", exc)
            return None, "", str(exc)

    def _heartbeat_evaluation_prompt(self) -> str:
        return render_identity_template(HEARTBEAT_EVALUATION_PROMPT_TEMPLATE, self.identity)

    def _extract_heartbeat_fallback(self, raw_response: str) -> dict | None:
        if not raw_response:
            return None
        mood_match = re.search(r'"mood"\s*:\s*"([^"]+)"', raw_response)
        confidence_match = re.search(r'"confidence"\s*:\s*([\d.]+)', raw_response)
        summary_match = re.search(r'"summary"\s*:\s*"([^"]*)"', raw_response)
        if not mood_match and not summary_match:
            return None
        mood = mood_match.group(1) if mood_match else "warm_neutral"
        confidence = float(confidence_match.group(1)) if confidence_match else 0.5
        summary = summary_match.group(1) if summary_match else ""
        return {
            "event_type": "summary_heartbeat",
            "perceived_intent": summary[:200] if summary else f"heartbeat mood: {mood}",
            "affect_delta": {key: 0.0 for key in self.AFFECT_KEYS},
            "relationship_event": False,
            ...
            "mood_label": mood[:60],
            "reply_guidance": "",
            "residue": summary[:500] if summary else "",
            "confidence": self._clamp_float(confidence, 0.0, 1.0),
        }

    def _heartbeat_hash(self, session_id: str, summary_text: str) -> str:
        text = "\n".join([self.profile_id, session_id, "heartbeat", summary_text])
