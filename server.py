...
async def api_heartbeat(request):
    """Accept persona heartbeat from external plugins (e.g. persona_heartbeat Operit plugin).

    Validates X-Heartbeat-Key header, extracts conversation summary from the
    plugin's messages, then delegates to persona_engine.process_heartbeat()
    which uses the internal evaluator to produce a full structured assessment.
    """
    from starlette.responses import JSONResponse

    heartbeat_key = persona_engine.heartbeat_key
    if heartbeat_key:
        provided_key = (request.headers.get("X-Heartbeat-Key") or "").strip()
        if not provided_key or not hmac.compare_digest(provided_key, heartbeat_key):
            return JSONResponse({"error": "invalid heartbeat key"}, status_code=401)
    ...
    session_id = str(body.get("session_id") or "").strip() or "heartbeat_default"
    ...
    if not summary_text.strip():
        return JSONResponse({"error": "no user message content found in messages"}, status_code=400)

    try:
        result = await persona_engine.process_heartbeat(session_id, summary_text)
        return JSONResponse({
            "ok": True,
            "session_id": session_id,
            "mood_label": result.get("affect", {}).get("mood_label", ""),
            "residue": result.get("affect", {}).get("residue", ""),
        })
    except Exception as e:
        logger.exception("Heartbeat processing failed: %s", e)
        return JSONResponse({"error": str(e)}, status_code=500)
