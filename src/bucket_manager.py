...
class BucketManager:
    ...
    async def search(
        ...
        owner_filter_set: Optional[set] = None,
    ) -> list[dict]:
        """
        ...
        if not _effective_owner_set:
            ...
            if _ctx_owner:
                _effective_owner_set = parse_owner_param(_ctx_owner)
        if _effective_owner_set:
            candidates = [
                b for b in candidates
                if bucket_matches_owner(b["metadata"], _effective_owner_set)
            ]
