...
from owner_filter import set_current_owner, reset_current_owner
...
# 插件：初始化 reranker_engine（非 upstream，独立模块）
try:
    from reranker_engine import RerankerEngine
    reranker_engine = RerankerEngine(config)
except Exception as _rr_err:
    logger.warning(f"[plugin] reranker_engine init failed: {_rr_err}")
...
async def hold(
    ...
    """存入一条记忆(一句话级)。系统自动打标并尝试与近似的已有桶合并。tags 逗号分隔,importance 1-10。pinned=True=标记为永久核心,不衰减不合并。feel=True=存为感受类记忆(不参与普通浮现,仅通过 breath(domain=\"feel\") 读取)。source_bucket=正在消化的原始记忆桶 ID,会被标为已消化以加速淡化。why_remembered=记录原因(可选,自由文本,仅用于展示不计分)。owner=记忆归属(alove/pearl/shared),留空=不过滤(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
            _t_hold.dispatch(
...
@mcp.tool()
async def grow(content: str, owner: Optional[str] = "") -> str:
    """整理一段长文本(如一天的记录/一段日记/一篇总结)存入记忆,系统拆分为 2~6 条独立事件桶并各自尝试合并。短内容(<30 字)走 hold 单条快速路径,不强行拆分。owner=记忆归属(alove/pearl/shared),留空=不过滤(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
            _t_grow.dispatch(content),
            op="grow",
            args={"content_len": len(content or ""), "owner": owner},
        )
    finally:
        reset_current_owner(_owner_token)
...
async def plan(
    ...
    """登记一个待办/承诺/未闭环事项。status=active(默认)/resolved/abandoned。related_bucket 可选,关联到某个普通记忆桶。weight=承诺重量 0.0-1.0(默认 0.5),与 importance 区分——importance 表示「多重要」、weight 表示「多重」。why_remembered=登记原因(可选、仅展示)。plan 不衰减、不出现在普通 breath,仅在 dream 末尾的 active 段返回;后续 hold/grow 写入新事件时系统自动判断已登记的 plan 是否完成。owner=记忆归属(alove/pearl/shared),留空=不过滤(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
            _t_plan.plan_create(
...
async def letter_write(
    ...
    """写入一封信。author 必填:\"user\"=用户一方写的,\"ai\"(或等于 ai_name)=AI 一方写的,也可直接传任意署名字符串;user_name 可选;ai_name 可选(默认取环境变量 AI_NAME,回退 \"AI\");title/date 可选。信件原文永久保存,不压缩/不合并/不衰减,仅建向量索引;普通 breath 不返回,SessionStart 钩子会带上双方各最新一封。owner=信件归属(alove/pearl/shared),留空=shared(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
...
async def letter_read(
    ...
    """检索历史信件。query=语义检索(可选);author 按署名过滤(\"user\"=用户侧,\"ai\"=AI 侧,也可传具体署名字符串);date_from/date_to=ISO 日期范围(可选)。无 query 时按时间倒序返回最近 limit 封。返回完整原文,不压缩。owner=信件归属(alove/pearl/shared),留空=不过滤(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
            _t_plan.letter_read(
...
async def dream(window_hours: Optional[int] = 48, owner: Optional[str] = "") -> str:
    ...
    owner=记忆归属(alove/pearl/shared),留空=不过滤(向后兼容)。"""
    _owner_token = set_current_owner(owner if (owner and owner.strip()) else None)
    try:
        return await _with_notice(
