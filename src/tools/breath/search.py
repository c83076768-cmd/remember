...
async def surface_search(
    ...
    try:
        rr_engine = getattr(rt, "reranker_engine", None)
        if rr_engine and rr_engine.enabled and len(matches) > 1:
            candidate_limit = getattr(rr_engine, "candidate_limit", 40)
            candidates = matches[:candidate_limit]
            documents = []
            for b in candidates:
                content = b.get("content", "") or ""
                meta = b.get("metadata", {}) or {}
                documents.append(f"{meta.get('name', b['id'])}\n{content[:500]}")
            rr_results = await rr_engine.rerank(query, documents, top_n=len(candidates))
            if rr_results:
                score_weight = getattr(rr_engine, "score_weight", 0.65)
                reranked = []
                for r in rr_results:
                    if 0 <= r.index < len(candidates):
                        orig = candidates[r.index]
                        orig_score = float(orig.get("score", 0) or 0)
                        blended = orig_score * (1 - score_weight) + r.score * 100 * score_weight
                        orig["score"] = round(blended, 2)
                        orig["rerank_score"] = round(r.score, 4)
                        reranked.append(orig)
                if reranked:
                    matches = reranked + matches[len(candidates):]
                    rt.logger.info(f"[reranker] reranked {len(reranked)} candidates")
