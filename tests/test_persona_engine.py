...
@pytest.mark.asyncio
async def test_persona_heartbeat_records_event_with_internal_evaluator(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(_event_payload(
        event_type="affection",
        mood_label="warm_touched",
    ...
    state = await engine.process_heartbeat("session-hb-1", "用户说爱你，AI回应也很温柔")

    assert state["affect"]["mood_label"] == "warm_touched"
    assert state["affect"]["residue"] == "心跳余味"
    assert _event_count(engine.db_path) == 1

    conn = sqlite3.connect(engine.db_path)
    row = conn.execute("SELECT event_type, perceived_intent FROM persona_events").fetchone()
    conn.close()
    assert row[0] == "affection"


@pytest.mark.asyncio
async def test_persona_heartbeat_idempotent(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(_event_payload())

    first = await engine.process_heartbeat("session-hb-idem", "同样的摘要内容")
    second = await engine.process_heartbeat("session-hb-idem", "同样的摘要内容")
...
@pytest.mark.asyncio
async def test_persona_heartbeat_empty_summary_no_event(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(_event_payload())

    state = await engine.process_heartbeat("session-hb-empty", "")
...
@pytest.mark.asyncio
async def test_persona_heartbeat_truncated_json_uses_regex_fallback(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(
        '{"mood":"cooperative","confidence":0.75,"summary   '
    )

    state = await engine.process_heartbeat("session-hb-trunc", "摘要内容被截断测试")

    assert _event_count(engine.db_path) == 1

    conn = sqlite3.connect(engine.db_path)
    row = conn.execute("SELECT event_type, mood_label, perceived_intent FROM persona_events").fetchone()
    conn.close()
    assert row[0] == "summary_heartbeat"
    assert row[1] == "cooperative"


@pytest.mark.asyncio
async def test_persona_heartbeat_malformed_json_no_match_records_error(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient("completely not json at all")

    state = await engine.process_heartbeat("session-hb-bad", "完全无法解析的内容")

    assert _event_count(engine.db_path) == 1

    conn = sqlite3.connect(engine.db_path)
    row = conn.execute("SELECT error FROM persona_events").fetchone()
    conn.close()
    assert row[0] is not None


def test_persona_heartbeat_key_from_config(test_config):
    cfg = _persona_config(test_config, heartbeat_key="test_key_123")
    engine = PersonaStateEngine(cfg)
    assert engine.heartbeat_key == "test_key_123"


def test_persona_heartbeat_key_from_env(test_config, monkeypatch):
    monkeypatch.setenv("OMBRE_HEARTBEAT_KEY", "env_key_456")
    cfg = _persona_config(test_config, heartbeat_key="config_key")
    engine = PersonaStateEngine(cfg)
    assert engine.heartbeat_key == "env_key_456"


@pytest.mark.asyncio
async def test_persona_heartbeat_dashboard_shows_events(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(_event_payload(
        event_type="affection",
        mood_label="warm_touched",
    ...
    await engine.process_heartbeat("session-hb-dash", "Dashboard 可见性测试")
    payload = engine.get_dashboard_payload(session_id="session-hb-dash")
...
@pytest.mark.asyncio
async def test_persona_heartbeat_client_status_lines_stripped(test_config):
    engine = PersonaStateEngine(_persona_config(test_config))
    engine.client = FakePersonaClient(_event_payload())

    await engine.process_heartbeat(
        "session-hb-status",
        "想你了\n当前时间：2026-05-25 12:30:00\n电量：100%",
    )

    if engine.client.calls:
        payload = json.loads(engine.client.calls[0]["messages"][1]["content"])
        assert "当前时间" not in payload["conversation_summary"]
        assert "想你了" in payload["conversation_summary"]
